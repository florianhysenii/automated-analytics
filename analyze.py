...
def get_table_schema(sql_query_tool, sql_engine="mysql"):
    ...
    if sql_engine == "sqlserver":
        ...
        raise Exception(
            "unsupported SQL engine, please manually update code to retrieve database schema"
        )
    ...
    if columns:  # Ensure there's something to add for the last table
        output.append(f"table: {current_table}, columns: {', '.join(columns)}")
...
class SQL_Query(ChatGPT_Handler):
    def __init__(
        ...
        **kwargs,
        ...
        self.db_path = db_path  # This is for SQLite

        self.driver = driver

    def execute_sql_query(self, query, limit=10000):
        if self.db_path is not None:  # SQLite path
            engine = create_engine(f"sqlite:///{self.db_path}")
        else:  # MySQL connection
            engine = create_engine(
                f"mysql+mysqlconnector://{self.db_user}:{self.db_password}@{self.dbserver}/{self.database}"
            )
