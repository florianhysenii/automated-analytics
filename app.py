...
load_setting("SQL_ENGINE" ,"sqlengine", "mysql")
...
load_setting("MYSQL_SERVER", "mysqlserver")
load_setting("MYSQL_DATABASE", "mysqldatabase")
load_setting("MYSQL_USER", "mysqluser")
load_setting("MYSQL_PASSWORD", "mysqlpassword")
...
with st.sidebar:  
    ...
    if st.session_state['show_settings']:  
        ...
        with st.form("AzureOpenAI"):
            ...
            st.radio("SQL Engine:",["sqlserver", "mysql"],index=0,key="txtSQLEngine")
            st.write("SQL Server Settings (Optional)")
            st.text_input("Server:", value=st.session_state.sqlserver,key="txtSQLServer")  
    ...
    if st.button("Submit"):
        ...
        if st.session_state.sqlengine == "sqlserver":
            sql_query_tool = SQL_Query(
                dbserver=st.session_state.sqlserver,
                database=st.session_state.sqldatabase,
                db_user=st.session_state.sqluser,
                db_password=st.session_state.sqlpassword
            )
        elif st.session_state.sqlengine == "mysql":
            sql_query_tool = SQL_Query(
                dbserver=st.session_state.sqlserver,
                database=st.session_state.sqldatabase,
                db_user=st.session_state.sqluser,
        ...
        if sql_query_tool is not None:
            analyzer = AnalyzeGPT(
                sql_engine=st.session_state.sqlengine,
